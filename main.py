...
if __name__ == '__main__':
    ...
    parser.add_argument('-net', type=str, default="resnet20", help='net')
    parser.add_argument('-data', type=str, default="CIFAR10", help='CIFAR10 or CIFAR100 or ImageNet-200')
    ...
    parser.add_argument('-save_path', type=str, default="./result/mean.csv", help='save path')
    ...
    print(index_ori)
    print(index_ori.mean())
    mean = index_ori.mean()
    # exit(0)
    basemap_ori = get_basemap(bucket, index_ori, images_ori, labels_ori)
    print(basemap_ori[1])
    print(basemap_ori[1][args.bucket - 1])
    if ((args.index == "confidence") | (args.index == "margin")):
        lastnum = basemap_ori[1][0]
    else:
        lastnum = basemap_ori[1][args.bucket - 1]
    csv_writer.writerow(["bucket", args.bucket, "index", args.index, "data", args.data, "net", args.net, "mean", mean])
    # exit(0)
    ...
    #         basemap_ood = get_basemap(bucket, index_ood, images_ood, labels_ood)
    #         if ((args.index == "confidence") | (args.index == "margin")):
    #             pred_acc_atc = np.count_nonzero(-index_ood >= thres_atc) / len(index_ood)
    #         else:
    #             pred_acc_atc = np.count_nonzero(index_ood >= thres_atc) / len(index_ood)
    #         print(basemap_ood[1])
    #         # 计算ood数据集的acc 
    ...
    #             acc2 = (basemap_ood[1][-1:].sum() / basemap_ori[1][-1:].sum()) * base_acc
    #         else:
    #             acc2 = (basemap_ood[1][:1].sum() / basemap_ori[1][:1].sum()) * base_acc
